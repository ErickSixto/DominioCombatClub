...
export default function HomePage() {
  ...
  return (
    <div data-testid="home-page">
      <section data-testid="hero-section" className="relative min-h-[92vh] flex items-end pb-20 md:pb-28 bg-dark-warm">
        <img
          src={IMAGES.hero}
          ...
          <div className="max-w-2xl">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4 opacity-0 animate-fade-in-up animation-delay-300">
              <a
                ...
                className="btn-gold btn-primary btn-glow"
              >
              ...
              <Link
                ...
                className="btn-primary border border-white/30 text-[#F4F3EF] hover:bg-white/10 hover:border-white/50 transition-all duration-300"
              >
                {tx.ctaSecondary}
      ...
      <section data-testid="trust-bar" className="bg-cream">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-12 md:gap-16">
            <AnimateIn delay={0}>
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading text-lg font-bold text-[#1A1720]">5.0</span>
                <span className="text-[#3A3740] text-sm">({lang === 'es' ? '104 resenas' : '104 reviews'})</span>
              </div>
            </AnimateIn>
            <div className="hidden sm:block w-px h-6 bg-[#1A1720]/10" />
            <AnimateIn delay={150}>
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-[#C9A24A]" />
                <span className="text-[#3A3740] text-sm">{tx.trustSince} <span className="text-[#1A1720] font-bold">2016</span></span>
              </div>
            </AnimateIn>
            <div className="hidden sm:block w-px h-6 bg-[#1A1720]/10" />
            <AnimateIn delay={300}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#3A3740] text-sm">{tx.trustFree} <span className="text-[#1A1720] font-bold">{tx.trustFreeB}</span></span>
              </div>
            </AnimateIn>
          </div>
        </div>
      ...
      <section data-testid="programs-section" className="bg-dark-rich glow-gold-top py-24 md:py-36">
            ...
            {progs.map((prog, i) => (
              <AnimateIn key={prog.name} delay={i * 100} className={i === 0 ? "sm:col-span-2 lg:col-span-2" : ""}>
                <Link
                  ...
                  className="program-card card-gradient-border group relative overflow-hidden bg-[#1A1720] border border-white/5 h-72 md:h-80 flex flex-col justify-end"
                >
                  <img
                    src={prog.image}
                    alt={prog.name}
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0F0D10] via-[#0F0D10]/40 to-transparent" />
      ...
      <section data-testid="methodology-section" className="bg-cream-warm py-24 md:py-36">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <AnimateIn>
              <span className="font-heading text-sm uppercase tracking-[0.2em] text-[#C9A24A]">{tx.methLabel}</span>
              <h2 className="font-heading text-3xl md:text-5xl font-bold uppercase tracking-tight text-[#1A1720] mt-3">
                {tx.methTitle}
              </h2>
                ...
                {[ShieldCheck, Users, Trophy].map((Icon, idx) => (
                  <AnimateIn key={idx} delay={idx * 150}>
                    <div className="card-light p-5 flex gap-4">
                      <div className="w-10 h-10 rounded-full bg-[#C9A24A]/10 flex items-center justify-center shrink-0">
                        <Icon className="w-5 h-5 text-[#C9A24A]" />
                      </div>
                      <div>
                        <h3 className="font-heading text-lg uppercase font-semibold text-[#1A1720]">{tx.methItems[idx].title}</h3>
                        <p className="text-sm leading-relaxed mt-1 text-[#3A3740]">{tx.methItems[idx].text}</p>
                      </div>
                    </div>
                  </AnimateIn>
                ))}
            ...
            <AnimateIn delay={200}>
              <div className="relative">
                ...
                <div className="absolute -bottom-6 -left-6 bg-[#C9A24A] p-6 hidden md:block animate-float">
                  <p className="font-heading text-3xl font-bold text-[#0F0D10]">+9</p>
                  <p className="font-heading text-sm uppercase tracking-wider text-[#0F0D10]">{tx.methYears}<br />{tx.methYearsB}</p>
                </div>
              </div>
            </AnimateIn>
          </div>
      ...
      <section data-testid="schedule-snapshot" className="bg-dark-rich py-24 md:py-36">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimateIn>
          ...
          <AnimateIn delay={150}>
            <div className="overflow-x-auto glass-card rounded-sm">
              <table className="w-full min-w-[500px]">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left font-heading text-sm uppercase tracking-wider text-[#B9B4A8] py-4 px-6">{lang === 'es' ? 'Disciplina' : 'Discipline'}</th>
                    <th className="text-left font-heading text-sm uppercase tracking-wider text-[#B9B4A8] py-4 px-6">
                ...
                <tbody>
                  {sched.map((row) => (
                    <tr key={row.discipline} className="border-b border-white/5 schedule-cell hover:bg-white/[0.03] hover:shadow-[inset_0_0_20px_rgba(201,162,74,0.05)] transition-all duration-300">
                      ...
                      <td className="py-4 px-6 text-sm text-[#B9B4A8]">{row.morning}</td>
                      <td className="py-4 px-6 text-sm text-[#B9B4A8]">{row.evening}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section data-testid="pricing-teaser" className="bg-gold-dark glow-gold-top py-24 md:py-36">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-5xl mx-auto">
            <AnimateIn delay={0}>
              <div data-testid="pricing-card-1" className="glass-card glass-card-hover p-7 flex flex-col h-full">
            ...
            <AnimateIn delay={150}>
              <div data-testid="pricing-card-2" className="glass-card glass-card-hover border-2 animate-border-glow p-7 flex flex-col relative overflow-hidden h-full md:-mt-3 md:mb-[-0.75rem]">
                <div className="absolute top-0 right-0 bg-[#C9A24A] text-[#0F0D10] font-heading text-xs uppercase tracking-wider font-bold px-3 py-1">
                  {tx.plan2Badge}
                </div>
                ...
                <div className="mt-5 mb-5">
                  <span className="font-heading text-3xl font-bold text-gradient-gold">$1,300</span>
                  <span className="text-[#B9B4A8] text-xs ml-2">MXN / {lang === 'es' ? 'mes' : 'month'}</span>
                </div>
                <ul className="space-y-2.5 mb-6 flex-1">
                ...
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
                  className="btn-gold btn-primary text-xs tracking-wider px-5 py-3">
                  {tx.planCta}
                </a>
              </div>
            </AnimateIn>
            <AnimateIn delay={300}>
              <div data-testid="pricing-card-3" className="glass-card glass-card-hover p-7 flex flex-col relative overflow-hidden h-full">
          ...
          <p className="text-center text-[#B9B4A8]/60 text-xs mt-8">{tx.inscNote}</p>
      ...
      <section data-testid="testimonials-section" className="bg-cream py-24 md:py-36">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimateIn className="text-center mb-12 md:mb-16">
            ...
            <div className="flex items-center justify-center gap-2 mt-4">
              ...
              <span className="text-[#1A1720] font-bold">5.0</span>
              <span className="text-[#3A3740] text-sm">{tx.reviewsGoogle} (104 {tx.trustReviews})</span>
            </div>
          </AnimateIn>
            ...
            {testimonials.map((review, i) => (
              <AnimateIn key={review.name} delay={i * 100}>
                <div
                  ...
                  className="card-light p-6 h-full"
                >
                  ...
                  <blockquote className="text-[#3A3740] text-sm leading-relaxed mb-4">"{review.text[lang]}"</blockquote>
                  <p className="font-heading text-sm uppercase tracking-wider text-[#1A1720] font-semibold">{review.name}</p>
                </div>
              </AnimateIn>
            ))}
          </div>
      ...
      <section data-testid="location-section" className="bg-dark-warm py-24 md:py-36">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <AnimateIn>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-sm text-[#F4F3EF]">C. 32A 514, Maya, 97134</p>
                    <p className="text-sm text-[#B9B4A8]">Merida, Yucatan, Mexico</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <Clock className="w-5 h-5 shrink-0 text-[#C9A24A] mt-0.5" />
                  <div>
                    <p className="font-semibold text-sm text-[#F4F3EF]">{tx.locDays}</p>
                    <p className="text-sm text-[#B9B4A8]">{tx.locHours}</p>
                  </div>
                </div>
              </div>
              <div className="mt-8 flex flex-col sm:flex-row gap-4">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="btn-primary border border-white/20 text-[#F4F3EF] hover:bg-white/10 hover:border-white/40 transition-all duration-300"
                >
                  <MessageCircle className="w-4 h-4" />
                  WhatsApp
                </a>
            ...
            <AnimateIn from="right">
              <div className="map-container h-[350px] md:h-[400px] bg-[#1A1720] rounded-sm overflow-hidden">
                <iframe
      ...
      <section data-testid="final-cta" className="bg-gold-dark glow-gold-top py-24 md:py-36 text-center relative">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimateIn>
            <p className="font-heading text-sm uppercase tracking-[0.3em] mb-6 italic text-[#B9B4A8]">
              "<span className="text-gradient-gold">{tx.finalMotto}</span>"
            </p>
            <h2 className="font-heading text-3xl md:text-5xl lg:text-6xl font-bold uppercase tracking-tight text-[#F4F3EF]">
              {tx.finalTitle}
            </h2>
            <p className="mt-5 text-[#B9B4A8] text-base md:text-lg max-w-lg mx-auto leading-relaxed">
              {tx.finalSub}
            </p>
            <a
              href={WHATSAPP_URL}
              ...
              className="btn-glow mt-10 inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#128C7E] text-white font-bold text-sm uppercase tracking-wider px-10 py-4 rounded-sm transition-all duration-300 hover:scale-105"
            >
